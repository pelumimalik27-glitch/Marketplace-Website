...
function BuyerMessages() {
    ...
    return conversations
      .map((conversation) => {
        const participants = Array.isArray(conversation?.participants)
          ...
          : [];
        const partner =
          participants.find((participant) => asId(participant) !== currentUserId) || null;
        const partnerId = asId(partner) || "";
  ...
  }, [conversations, search, user?.userId]);
  ...
  useEffect(() => {
    ...
    const onConversation = ({ conversation } = {}) => {
      ...
      const participants = Array.isArray(conversation?.participants)
        ...
        : [];
      if (!participants.includes(String(user.userId))) return;
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="grid min-h-0 grid-cols-1 gap-4 lg:h-[calc(100vh-14rem)] lg:grid-cols-[340px_1fr]">
        <section className="flex min-h-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg">
          <div className="border-b p-4">
            <h1 className="text-lg font-semibold text-slate-900">My Messages</h1>
            <p className="text-xs text-slate-500">Chat with sellers in real time</p>
